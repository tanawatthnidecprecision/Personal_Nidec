...
class Paperless {
  ...
  async getQuery(table, raw){
    const response = await fetch(
      this.base_url +
        this.service +
        `?table_name=${table}&query_raw=${raw}`
    );
    const data_response = await response.json();
    if (data_response) {
      return data_response;
...
class IO extends Service {
  ...
  async uploadFile(file) {
    ...
    const response = await fetch(
      this.base_url + this.service + "upload/",
      {
        method: "POST",
        body: formData,
      }
  ...
  async getFile(path) {
    const response = await fetch(
      this.base_url + this.service + url + `upload/?image=${path}`
    );
    return response;
  }
}

class Supplier extends Paperless {
  constructor() {
    super();
  }

  getLikeName(value) {
    this.setService("api/name_service_supplier/");
    return this.getLike("name", value);
  }
}

class Production extends Service {
  constructor() {
    super("api/");
  }

  getCounterUpdateInput(number) {
    return this.get(
      `production/?database=counter_update_input&query_key=mc&query_value=${number}`
    );
  }

  getCounterUpdate(number) {
    return this.get(
      `production/?database=counter_update&query_key=mc&query_value=${number}`
    );
  }

  getCounterConfig(number) {
    return this.get(
      `production/?database=counter_config_input&query_key=mc&query_value=${number}`
    );
  }
}
